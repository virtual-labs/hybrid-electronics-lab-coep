(function(scope) {
	...
	utility.addBitByBitBinary = function(num1, num2, isArray) {
		var carry = 0;
		var index;
		...
		if(isArray) {
			for(index = arrayLength; index >= 0; index--) {
				var resultObject = addBits(num1[index], num2[index], carry);
				var sum = resultObject.result;
				resultnumber += sum;
				carry = resultObject.carry;
			}
		} else {
			var num1Array = num1.toString().split("");
			var num2Array = num2.toString().split("");
			for(index = 0; index <= arrayLength; index++) {
				var resultObject = addBits(parseInt(num1Array[index]), parseInt(num2Array[index]), carry);
				var sum = resultObject.result;
				resultnumber += sum;
				carry = resultObject.carry;
			}
		}
		resultObject.number = parseInt(resultnumber);
