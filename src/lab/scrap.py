...
def writefile(fname,s,s1,heading):
	...
	content=f.read()
	#print content
	filedata=content.replace('index.html','Theory.html?domain=ElectronicsandCommunications&lab='+heading)
	f.seek(0)
	f.write(filedata)	
...
print heading
breadcrumb='<a href="http://vlabs.ac.in/electronics-and-communications-labs.html" class="sidebar-a" >Electronics & Communications</a><br/>'
while sectionNumber<=len(sectionno):
	...
	if tag1=='Prerequisite S/W':
		tag1='Prerequisites'
	st+='<a href="'+tag1+'.html?domain=ElectronicsandCommunications"'+' class="sidebar-a" > <h3 class="text-h3-darkblue" style="margin-top: 2px;">'+tag1+'</h3></a>'	
	sectionNumber=sectionNumber+1
sectionNumber=1
while sectionNumber<=len(sectionno):
	tag=""
